...
import { getClients, createClient, updateClient, deleteClient } from '../services/api-clients';
    ...
    const handleCreateClient = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.mobile && !normalizedMobile) {
                toast.error('Número de teléfono inválido (usa +prefijo o un número local válido).');
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredClients.map(client => (
                                <tr key={client.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div>{client.createdAt ? new Date(client.createdAt).toLocaleString() : '-'}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex items-center justify-end space-x-2">
                                            <button onClick={() => openEditModal(client)} className="text-blue-600 hover:text-blue-900">
                            ...
                            {filteredClients.length === 0 && (
                                <tr>
                                    <td colSpan={4} className="px-6 py-4 text-center text-sm text-gray-500">No hay clientes</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
